...
async function create(req, res) {
  try {
    ...
    if (!participationId)
      return res.status(400).json({ message: 'participationId required' });

    const part = await pool.query(
      'SELECT id, event_id FROM participations WHERE id = $1',
      [participationId]
    );
    if (!part.rows[0])
    ...
    const eventRow = await pool.query(
      'SELECT cost FROM events WHERE id = $1',
      [part.rows[0].event_id]
    );
    const amount = eventRow.rows[0]?.cost ?? 0;

    await pool.query(
      'INSERT INTO payments (participation_id, transaction_id, screenshot, amount) VALUES ($1, $2, $3, $4)',
      [participationId, transactionId ?? null, screenshotPath, amount]
    );

    await pool.query(
      "UPDATE participations SET payment_status = 'paid' WHERE id = $1",
      [participationId]
    );
